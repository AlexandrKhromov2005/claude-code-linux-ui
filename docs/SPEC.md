...
> **Ревизия 2026-08-19.** Продукт стал только веб-клиентом: TUI удалён, бинарь
> без аргументов поднимает локальный веб-сервер (см. README). TUI-специфика
> ниже — стек Bubble Tea в ADR-2, экраны и хоткеи §6 — историческая база
> дизайна. Ядро, модель данных, обвязка CLI и политика безопасности (§3–§5,
> §7, §10) действуют как написано.
